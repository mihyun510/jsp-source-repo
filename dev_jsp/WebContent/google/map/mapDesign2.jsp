<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
    <%
    	//html보다 무조건 먼저 실행 위치가 어디가 되었든
    	//필요한 정보 로그인이 완료되면 나오는 사용자의 이름.
    	Cookie cs[] = request.getCookies();
    	String c_name = null;
	    for(int i=0; i<cs.length; i++){
			out.print(cs[i].getName() +", "+cs[i].getValue()+"<br>");
			//JSessionID, c_name, A432423432, 이순신  -> 이 값들을 가져오자.
			c_name = "이순신";
		}
	%>
<!DOCTYPE html>
		...
		function login() {
			...
			fetch("./loginAction.jsp?mem_id="+u_id+"&mem_pw="+u_pw,{
				...
				if("비밀번호가 틀립니다."==text.trim()){ //비밀번호가 틀립니다.
					...
					c_name = text.trim();//여기에 이름을 담는다. 
					/*$.cookie('c_name',c_name); ---> 로그인 액션에서 이루어져야 된다. 여기는 클라이언트에서 가져온 값*/
					$.cookie('c_name','<%=c_name%>'); //서버에서 가져온다.
					$("#d_login").hide();
					$("#d_logout").show();
